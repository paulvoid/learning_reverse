...
namespace ProcUtil
{
    ...
    std::uintptr_t Scan(HANDLE process,std::uintptr_t BaseAddress,std::uintptr_t VFTableAddress);
	bool IsOS64Bit();
    ...
    uintptr_t GetDMAAddress(HANDLE process,uintptr_t ptr, std::vector<unsigned int> offsets);

    DWORD GetPointerAddress(HANDLE process,DWORD address);

    std::vector<DWORD> GetChildren(HANDLE process, DWORD instance);


    std::string GetClassType(HANDLE process, DWORD instance);

    DWORD GetService(HANDLE process, DWORD game, std::string className);

    std::string ReadStringOfUnknownLength(HANDLE process, DWORD address);

    std::string GetName(HANDLE process, DWORD instance);

    DWORD FindFirstChild(HANDLE process, DWORD instance, std::string name);
}